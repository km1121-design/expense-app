...
レシート画像のAI読み取りと、個人／管理者向けの確認ダッシュボードを備えた
...
- **画像解析**: レシート／領収書写真から金額・日付・店名・科目・摘要をAIで自動入力
  （1回目で読めなかったレシートだけ、根拠を厚くして読み直す）
- **手修正の学習**: 手直しした内容を店舗ごとに記憶し、次回同じ店舗のレシートで自動補正
- **電車賃の照合**: 出発駅・到着駅・片道/往復・回数から想定運賃を出して申請額と
  突き合わせ。未登録の区間は申請額から運賃マスタへ自動登録され（最初の1件は
  「新規登録」として管理者が確認）、2回目以降の同じ区間は自動で照合
- **経費申請**: 日付・科目・支払先・金額・摘要を入力して申請
- **ログイン・権限**: ユーザーID＋パスワード認証。一般（自分の申請のみ）／管理者
  （全件・承認・ユーザー管理）の2ロール。申請者名はサーバー側で強制
- **領収書ビューア**: 一覧に領収書サムネイルを表示。クリックで拡大、← → で次の領収書へ。
  カード表示と表表示を切り替え可能
- **個人ダッシュボード**: 自分の申請一覧・状況・金額集計・パスワード変更
- **管理者ダッシュボード**: 全申請の承認／却下、承認待ち・科目別金額の集計、検索、
  領収書あり／なしの絞り込み、CSV書き出し、ユーザー管理（追加・無効化・PW再設定）
