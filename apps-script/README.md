...
**1区間あたり1回だけ精度に投資すれば済む**ので、次の設定が有効です。

| 設定 | 効果 | 副作用 |
| --- | --- | --- |
| `FARE_MODEL` = `gemini-pro-latest` | 乗換のある区間の経路判断が安定する | 遅くなる（初回の照合のみ） |
| `GEMINI_MODEL` = `gemini-pro-latest` | レシート解析の読み取り精度が上がる | 毎回遅くなる・無料枠の消費が早い |
| `GEMINI_THINKING` = `default` | レシート解析で思考を制限しない | 遅くなる |

最も確実なのは、**最初に使う区間だけ人が実際の運賃と突き合わせて運賃マスタを直す**
ことです。以降その区間は常に正しい値で照合されます。
