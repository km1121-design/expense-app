...
スクリプトプロパティ `FARE_WEB_LOOKUP` を `false` にすると、AIによるWeb照合を
使わず**運賃マスタだけで照合**します。**無料枠でWeb検索が使えない場合は、
初回の失敗時に自動でこの設定に切り替わります**（何度も失敗して待たされないため）。
課金を有効にしたら、このプロパティを削除すれば再びWeb照合を使います。
...
- 検索を使いたい場合は、Google Cloud プロジェクトで**課金を有効化**したうえで、
  スクリプトプロパティ `FARE_WEB_LOOKUP` を削除してください
