...
3. （任意）`GEMINI_MODEL` でモデルを固定。未設定なら現行の flash 系（`gemini-3.6-flash` → `gemini-3.5-flash` → `gemini-3.1-flash-lite`）を順に試します
...
モデルは `FARE_MODEL` で固定でき、未設定なら現行の flash 系を新しい順に試します。
