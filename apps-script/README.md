...
レシート画像の読み取りは **AI解析 → 手入力** の2段構えです:

1. **AI解析**（`GEMINI_API_KEY` または `ANTHROPIC_API_KEY` 設定時）。
   金額・日付・店名に加え、**経費科目の自動判定・摘要の自動生成**まで行います。
2. **手入力** — AIキーが未設定のとき、または読み取れなかったとき。
   読み取れなくても、画像を添付したまま申請そのものは進められます。
...
指定すると Claude 優先）。いずれも未設定の場合は自動入力を行わず、手入力になります。
...
2. 「運賃マスタと照合」を押すと、**登録済みの区間はその値で即照合**されます。
   未登録なら「🚉 路線検索で調べる」で運賃を確認して金額を手入力してください
...
吸収して照合します。運賃改定や誤りは、管理者ダッシュボードの「運賃マスタ」で
運賃欄を直接書き換えて修正してください（削除すると未登録に戻り、次に申請された
ときの金額で登録し直されます）。
...
区間の運賃を外部から自動で調べる機能はありません（Google検索グラウンディングは
Gemini APIの無料枠では使えず、運賃マスタが申請から育つようになって不要になりました）。
路線検索サイトの自動読み取りは各社の規約で禁止されているため実装しません。
...
2. その申請額から片道運賃を割り戻して運賃マスタへ登録され、申請には
   「🚃 新規登録」バッジが付く。**まだ誰も確認していない金額**なので、
   管理者一覧の運賃照合フィルタ「新規登録」で拾って承認前に確かめる
3. 以降その区間は自動で照合される

よく使う区間を先に入れておきたい場合は、管理者ダッシュボードの運賃マスタにある
「区間を手で追加する」から登録できます（照合方法に `手動（管理者名）` が入ります）。

### 精度を上げたいとき

運賃は一度確定すれば運賃マスタに残るため、**1区間あたり1回だけ確認すれば済みます**。

| 方法 | 精度 | 費用 |
| --- | --- | --- |
| **運賃マスタへ手で登録**（最も確実） | 100% | 無料 |
| 申請額からの自動登録＋管理者の確認（既定） | 申請者の入力しだい | 無料 |

レシート解析側の精度は `GEMINI_MODEL` を pro 系にする、`GEMINI_THINKING` を
`default` に戻す、`RECEIPT_RAW_TEXT_MAX_LINES` を増やす、で上がります
（いずれも毎回遅くなります）。読めなかった分だけ上位モデルへ回したい場合は、
上記「読めなかったときだけ上位モデルへ回す」を使ってください。
...
2. **誤読事例のフィードバック** … その店舗で過去に金額・日付・店名を
   誤読していた場合で、**今回も同じ値で読んだときに限り**、実際の誤りを
   プロンプトへ添えて **1 回だけ読み直します**（履歴があるだけでは読み直しません）。
